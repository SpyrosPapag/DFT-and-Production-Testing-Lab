...
*/
void GetRandompattern(number,array,nbit)
register int number; level array[]; int nbit;
{
   register int i, mask;

   if(nbit == WORDSIZE)
      for(i=0; i<number; i++) array[i]=(level)random();
   else {
      mask = ~(ALL1<<nbit);
      for(i=0; i<number; i++) array[i]=(level)random() & mask;
   }
}
...
*/
void GetPRandompattern(number,array)
register int number; level array[];
{
   register int i;
